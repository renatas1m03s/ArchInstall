...
PROGRAM_TITLE = 'Arch Linux Install Wizard - v0.1' 
...
def set_regional_settings():
	...
	subprocess.run(['sh', '-c','arch-chroot /mnt localectl set-keymap ' + KEYBOARD_CONFIG])
	...
	subprocess.run(['sh', '-c','echo KEYMAP=' + KEYBOARD_CONFIG + ' >> /mnt/etc/vconsole.conf'])
	subprocess.run(['sh', '-c','arch-chroot /mnt localectl'])
	subprocess.run(['sh', '-c','arch-chroot /mnt timedatectl'])
...
def set_pass(): # Create main user and set root password
	...
	subprocess.run(['sh', '-c','cp -r '+ os.path.dirname(__file__) + ' /mnt/home/' + USERNAME + '/'])
...
def main():
	STEP = 1
	position = list((0,0))
	selected_items= list(("phoenix",)) # to avoid every single letter as a item use double round-brackets and a coma at the end
				...
				match STEP:
					...
					case 5:
						...
						if proceed_or_abort == PROCEED_OR_ABORT[0]:
							...
							set_telaicons()
							set_plymouth()
							set_fish_functions() # This function correct home folder permissions - need to be the last one
							print('\nSetup finished...\n\nPress any key...\n\n')
							with term.cbreak(), term.hidden_cursor():
								key = term.inkey()
							STEP+=1
